...
, in float4  InstanceXtras   : TEXCOORD7    // TimeToLife, TimeBlendingOut, ...  
)
{
  V2P output = (V2P)0;
  ...
  float time_diff = GlobalWorldTime - TimeToLife;
  float ratio = saturate(time_diff / TimeBlendingOut);
  output.opacity = smoothstep( 1, 0, ratio );
  //output.opacity = 1;
  ...
  )
{
  ...
  float4 noise0 = txNoise.Sample(samLinear,float2(amount_of_x,amount_of_z) * 0.05 + float2(0,1) ) * 10. - 5;
  float a = sin(noise0.x);
  float b = cos(noise0.y);
  float4 albedo_color = txNormal.Sample(samLinear, float2(amount_of_x,amount_of_z) * 0.5 + float2(a,b));

  o_albedo.xyz = albedo_color * float4(0.5,0,0.5,1.0);
