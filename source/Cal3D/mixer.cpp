...
{
  prev_animation_time = m_animationTime;
...
void CalMixer::updateSkeleton()
{
  ...
  // MCV ---------------
  if (prev_animation_time >= m_animationTime) {
    prev_local_info_is_valid = false;
  }

  auto root_bone = vectorBone[0];
  auto root_pos = root_bone->getTranslation();

  if (prev_local_info_is_valid) {
    delta_root_pos = root_pos - prev_local_root_pos;
  }
  else {
    delta_root_pos = CalVector(0, 0, 0);
  }
  prev_local_root_pos = root_pos;
  prev_local_info_is_valid = true;

  // Remove local displacement in Z. Just for now.
  root_pos.z = 0.0f;
  delta_root_pos.x = 0.0f;
  delta_root_pos.y = 0.0f;

  // Convert local displacement to world oriented
  delta_root_pos *= world_rot;
